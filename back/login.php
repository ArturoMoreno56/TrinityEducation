...
{
    $obtener_id = 'SELECT Usuarios.idUsuario AS ID FROM Usuarios 
                   INNER JOIN Correos ON Usuarios.idUsuario = Correos.idUsuario
                   WHERE Correo = ?';
...
{
    $update_login = 'EXEC Ultimo_login ?';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $consultar_existencia_alumno = 'SELECT COUNT(*) AS CANT FROM Correos 
                                    INNER JOIN Alumnos ON Alumnos.idUsuario = Correos.idUsuario 
                                    WHERE Correo = ?';
    ...
    if ($existe_alumno == 1) {
        ...
        if (password_verify($clave_ingresada, $clave_original)) {
            $obtener_nombre = "SELECT Nombres FROM Correos 
                               INNER JOIN Alumnos ON Alumnos.idUsuario = Correos.idUsuario 
                               WHERE Correos.idUsuario = ?";
            $resultado = sqlsrv_query($con, $obtener_nombre, array($id_actualizar));
            ...
            header('Location:../nav_alumno/inicio_alumno.php');
            exit();
        } else {
    ...
    if ($existe_docente == 1) {
        ...
        if (password_verify($clave_ingresada, $clave_original)) {
            $obtener_nombre = "SELECT Nombres FROM Correos 
                               INNER JOIN Docentes ON Docentes.idUsuario = Correos.idUsuario 
                               WHERE Correos.idUsuario = ?";
            $resultado = sqlsrv_query($con, $obtener_nombre, array($id_actualizar));
            $fila = sqlsrv_fetch_array($resultado, SQLSRV_FETCH_ASSOC);
            $nombre_usuario = $fila['Nombres'];
    ...
    $consultar_existencia_admin = 'SELECT COUNT(*) AS CANT FROM Correos 
                                   INNER JOIN Usuarios ON Usuarios.idUsuario = Correos.idUsuario 
                                   WHERE Correo = ?';
    ...
    if ($existe_admin == 1) {
        ...
        if (password_verify($clave_ingresada, $clave_original)) {
            $obtener_nombre = "SELECT Nombres FROM Correos 
                               INNER JOIN Administradores ON Administradores.idUsuario = Correos.idUsuario 
                               WHERE Correos.idUsuario = ?";
            $resultado = sqlsrv_query($con, $obtener_nombre, array($id_actualizar));
            $fila = sqlsrv_fetch_array($resultado, SQLSRV_FETCH_ASSOC);
            $nombre_usuario = $fila['Nombres'];
            ...
            $_SESSION['IdUsuario'] = $id_actualizar;
            actualizar_login($con, $id_actualizar);
            header('Location:../nav_admin/inicio_admin.php');
            exit();
        } else {
            header('Location:../index.html?error=clave_erronea');
            exit();
